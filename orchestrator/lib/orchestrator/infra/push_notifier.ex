defmodule Orchestrator.Infra.PushNotifier do
  ...
  defp maybe_auth_token(headers, %{"token" => token}) when is_binary(token),
    do: [{"x-a2a-token", token} | headers]

  defp maybe_auth_token(headers, _cfg), do: headers

  defp maybe_hmac(headers, %{"hmacSecret" => secret}, body)
  ...
  defp maybe_jwt(headers, %{"jwtSecret" => secret} = cfg, body)
       when is_binary(secret) and secret != "" do
  ...
  defp base64url(map) when is_map(map),
